...
const Navbar = () => {
  ...
  const navItems = [
    ...
    { name: 'Account', pageLink: '/login', icon: User }
  ]

  const navigate = (link) => {
    console.log(`Navigating to: ${link}`)
    setActiveItem(navItems.find(item => item.pageLink === link)?.name || 'Home')
    setIsOpen(false)
  }
    ...
    <>
      ...
      <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${
        ...
        <div className="container mx-auto px-3 sm:px-4 lg:px-8 max-w-7xl">
          ...
          {/* Mobile Navigation */}
          <div className={`md:hidden lg:hidden transition-all duration-300 overflow-hidden ${
